...
export default function PilotBuilder({ onSubmit, onBack, initialValues = null }) {
  ...
  const submit = (event) => {
    ...
    onSubmit(PILOT_PREFIX + JSON.stringify({
      ...
      canva_bullets: bullets, email_to: "me",
    }));
  };
  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-card/80 p-5 space-y-4">
      <div>
        <h2 className="text-xl font-semibold">Four-app pilot</h2>
        <p className="text-sm text-muted-foreground mt-1">
          Use your exact content and event time. AURA will create a Google Doc, a calendar event,
          a Canva slide, and email the PDF to your connected Gmail account.
          You will review each external change.
        </p>
      </div>
      {input("doc_title", "Google Doc title")}
      <label className="flex flex-col gap-1 text-sm text-foreground/80">
        Google Doc text
