...
async def evaluate(url, *, samples=30, concurrencies=(1, 2, 4, 8), provider_delay=.025, model_delay=.05):
            ...
            for concurrency in concurrencies:
                for workload in WORKLOADS:
                    ...
                    async def one(index):
                        ...
                        async with factory() as session:
                            run = WorkflowRun(workspace_id=workspace, prompt="Read fixed forecast", status=RunStatus.running,
                        ...
                        async with factory() as session:
                            ...
                            rows.append({"passed": passed, "compile_ms": compiled_ms, "delivery_ms": elapsed,
                                "first_result_ms": (first-run.created_at).total_seconds()*1000 if first else None,
                    ...
                    ceiling = 5000 if concurrency <= 2 else 10000
                    reports.append({"workload": workload, "concurrency": concurrency, "samples": len(rows),
                        "passed": all(row["passed"] for row in rows) and p95 <= ceiling,
                        "successes": sum(row["passed"] for row in rows), "p50_ms": percentile([r["delivery_ms"] for r in rows], .5),
                        "p95_ms": p95, "p95_ceiling_ms": ceiling, "throughput_runs_per_second": len(rows)/duration,
    ...
    return {"passed": all(r["passed"] for r in reports), "evaluated_at": datetime.now(timezone.utc).isoformat(),
        ...
        "scope": "execution_engine_with_deterministic_provider_and_model_fixtures", "production_load_certified": False, "execution_locks_exercised": url.startswith("postgresql"),
        "provider_delay_ms": provider_delay*1000, "model_delay_ms": model_delay*1000, "workloads": reports}
...
def main():
    ...
    report = asyncio.run(evaluate(url, samples=args.samples, concurrencies=tuple(args.concurrency)))
    args.report.write_text(json.dumps(report, indent=2))
    print(json.dumps({"passed": report["passed"], "scope": report["scope"], "workloads": len(report["workloads"])}))
