...
def source_backed_presentation_template(
    prompt: str,
    inventory: list[dict],
) -> WorkflowPlan | None:
    """Build a capability-driven research-to-presentation graph.

    This adapter is subject-agnostic: accepted source receipts are composed into
    the provider's concrete presentation schema only after the reads complete.
    """
    requested = prompt.casefold()
    if "canva" not in requested:
        return None
    if not any(word in requested for word in ("presentation", "deck", "slide")):
        return None
    if not re.search(
        r"\b(?:official|public|source(?:s|\s+links?)?|citation(?:s)?|references?)\b",
        requested,
    ):
        return None
    if any(word in requested for word in ("email", "send", "publish", "post")) and not re.search(
        r"\b(?:do\s+not|don't|never|without)\s+(?:email|send|publish|post)\b",
        requested,
    ):
        return None

    research = _owner(inventory, {"web.search", "web.page.read"})
    presentation = _owner(inventory, {"canva.presentation.create"})
    if not research or not presentation:
        return None

    research_slug = str(research["slug"])
    presentation_slug = str(presentation["slug"])
    subject_match = re.search(
        r"\b(?:comparing|compare|about|on)\s+(.+?)(?=[.!?]|\binclude\b|$)",
        prompt,
        re.IGNORECASE,
    )
    subject = (
    ...
    plan = WorkflowPlan.model_validate(
        {
            "name": title,
            "interpretation": (
                "Research the requested official public sources, read the evidence, "
                "and create the requested populated Canva presentation."
            ),
            "steps": [
                {
                    "key": "search_public_sources",
                    "agent": "Public Research Agent",
                    "tool_slug": research_slug,
                    "operation": "web.search",
                    "arguments": {"query": query, "limit": 8},
                    "reason": "Find the requested official public source evidence.",
                ...
                },
                {
                    ...
                    "arguments": {"url": "{{steps.search_public_sources.results.0.url}}"},
                    "reason": "Read the first official source for the requested comparison.",
                    "expected_output": "Public page title, URL, text, and links.",
                ...
                },
                {
                    ...
                    "tool_slug": research_slug,
                    "operation": "web.page.read",
                    "arguments": {"url": "{{steps.search_public_sources.results.1.url}}"},
                    "reason": "Read the second official source for the requested comparison.",
                    "expected_output": "Public page title, URL, text, and links.",
                    "depends_on": ["search_public_sources"],
                    "required_evidence": ["public_page_content"],
                },
                {
                    "key": "read_source_three",
                    "agent": "Public Research Agent",
                    "tool_slug": research_slug,
                    "operation": "web.page.read",
                    "arguments": {"url": "{{steps.search_public_sources.results.2.url}}"},
                    "reason": "Read one additional official source when available.",
                    "expected_output": "Additional public page title, URL, text, and links.",
                    "depends_on": ["search_public_sources"],
                    "optional": True,
                    "required_evidence": ["public_page_content"],
                },
                        ...
                        "phases": [
                            {
                                ...
                                "title": "First subject",
                                "items": [
                                    "{{steps.read_source_one.text}}",
                                    "Source: {{steps.read_source_one.url}}",
                                ],
                            },
                            {
                                "period": "Source 2",
                                ...
                                "items": [
                                    "{{steps.read_source_two.text}}",
                                    "Source: {{steps.read_source_two.url}}",
                                ],
                            },
                            {
                                "period": "Comparison",
                                ...
                                "items": [
                                    "Compare the accepted facts from both source pages.",
                                    "Include both verified source links.",
                                ],
                            },
                        ],
                    },
                    "reason": (
                        "Create the requested populated presentation from accepted source "
                        "receipts; compose concise slide text before approval."
                    ),
                    "expected_output": "Verified Canva presentation job and design identity.",
                    "consequential": True,
                    "depends_on": ["read_source_one", "read_source_two"],
                    "required_evidence": ["dispatch_receipt", "populated_presentation"],
                },
            ],
            "result_contract": {
                "primary_step_key": "create_presentation",
                "completion_step_key": "create_presentation",
    ...
    plan.planning_artifacts = {
        "planner_recovery_mode": "audited_source_backed_presentation",
        "connection_requirements": [
            str(item["slug"])
            for item in (research, presentation)
            if not item.get("connected", True)
        ],
        "timings_ms": {"model": 0, "repair": 0, "total": 0},
    }
