...
from .models import AuditEvent, DispatchIntent, RunStatus, StepStatus
...
def verification_due(step, now=None):
    ...
    return not value or datetime.fromisoformat(value) <= (now or datetime.now(UTC))
...
async def defer_verification(session, run, step, now=None):
    ...
    if count >= 6 or (now - started).total_seconds() >= 600:
        step.output = {
            **step.output,
            "outcome_check": {
                **step.output["outcome_check"],
                "status": "unverified",
                "reasons": [
                    "Provider job did not finish within the verification budget; its saved receipt is preserved"
                ],
            },
        }
        return False
    ...
    step.output = {
        **step.output,
        "verification_poll_count": count + 1,
        "verification_started_at": started.isoformat(),
        "verification_retry_at": available.isoformat(),
    }
    step.status = StepStatus.running
    transition_run(
        run,
        RunStatus.running,
        reason="verification_poll_deferred",
        actor="outcome-checker",
        phase="verification",
        supervisor_status="recovering",
        error=None,
        dispatch=None,
        metadata={"step_id": step.id, "poll": count + 1},
        allow_same=True,
    )
    ...
    if existing:
        ...
        session.add(
            DispatchIntent(
                workspace_id=run.workspace_id, run_id=run.id, kind="execute", available_at=available
            )
        )
    session.add(
        AuditEvent(
            ...
            run_id=run.id,
            actor="outcome-checker",
            event_type="step.verification_deferred",
            payload={"step_id": step.id, "poll": count + 1, "available_at": available.isoformat()},
        )
    )
