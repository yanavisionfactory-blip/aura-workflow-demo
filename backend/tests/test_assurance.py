...
def signed_fixture():
    tool = SimpleNamespace(
        id="tool", slug="notion", config={"connection_id": "dedicated"}, base_url=None
    )
    module = next(
    ...
    report = {
        ...
        "contract_hash": module["reliability"]["hash"],
        "scenarios": {
            s: "passed" for s in ("execute", "read_back", "receipt_resume", "lost_response")
        },
...
@pytest.mark.parametrize(
    "change", ["signature", "workspace", "connection", "contract", "expired", "scenario"]
)
def test_certification_rejects_untrusted_stale_or_incomplete_evidence(change):
    ...
    if change == "workspace":
        report["workspace_id"] = "other"
    if change == "connection":
        report["connection_fingerprint"] = "old"
    if change == "contract":
        report["contract_hash"] = "old"
    if change == "expired":
        report["expires_at"] = report["issued_at"]
    if change == "scenario":
        del report["scenarios"]["lost_response"]
...
async def test_notion_pagination_and_nested_reads_preserve_evidence():
    ...
    async def request(method, path, params):
        ...
        if path == "blocks/root/children" and not params.get("start_cursor"):
            return {
                "results": [{"id": "a", "has_children": True}],
                "has_more": True,
                "next_cursor": "next",
            }
...
def test_performance_gate_requires_samples_and_reports_slow_tail():
    assert (
        evaluate_performance([{"planning_time_ms": 10}], {"planning_time_ms": 20})["status"]
        == "insufficient_data"
    )
...
def test_uncertain_write_diagnostic_never_recommends_retry():
    run = SimpleNamespace(
        id="r", error="Previous action outcome is uncertain", status=RunStatus.waiting_for_action
    )
...
async def test_readiness_changes_with_permission_revocation_and_expiry(database):
    ...
    async with database() as session:
        ...
        session.add(cert)
        await session.commit()
        assert (await operation_readiness(session, "w", tool, "notion.page.update"))[
            "execution_ready"
        ]
...
async def test_probe_yields_only_its_own_checkpoint_and_resumes_without_replay(
    ...
    async def synth(*args):
        return UnifiedDeliverable(summary="Read", deliverable="Berlin 2026-09-09 from both reads")
    ...
    async with database() as session:
        ...
        transition_run(
            ...
            reason="test_fixture_process_restart",
            actor="test",
            dispatch=None,
        )
        session.add(
...
@pytest.mark.skipif(
    not os.getenv("AURA_TEST_POSTGRES_URL"), reason="Requires PostgreSQL recovery ownership"
)
async def test_postgres_canary_recovers_at_its_private_deadline_and_preserves_guards(monkeypatch):
    ...
    async def synth(*args):
        return UnifiedDeliverable(summary="Read", deliverable="Both forecasts retrieved")
...
async def test_jira_collection_contracts_and_continuation_are_preserved(monkeypatch):
    ...
    assert incomplete_evidence(
        "jira.projects.list", {"values": [], "isLast": False}, ["complete_collection"]
    )


async def test_injected_lost_response_reconciles_witness_without_second_write(
    ...
    assert set(json.loads(report.read_text())["cases"][0]["passed_scenarios"]) == {
        "lost_response",
        "read_back",
    }
...
def test_dispatch_receipts_cannot_supply_completed_outcome_evidence():
    ...
    module = next(
        m
        for m in native_manifest("tiktok")["capabilities"]
        if m["name"] == "tiktok.video.publish.init"
    )
    ...
    assert output_errors(
        module["name"], {"data": {"publish_id": "p"}, "error": {"code": "access_denied"}}
    )
