...
from app import main, orchestrator, pipedream_connect
...
async def broker_api(monkeypatch):
    ...
    async def certify(session, client, app_definition):
        return await certify_app(session, client, app_definition, config)
