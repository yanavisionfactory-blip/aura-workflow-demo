...
async def test_automatic_replanning_rejects_an_identical_read_repair(runtime, monkeypatch):
    monkeypatch.setattr(replanning, "SessionLocal", runtime)
    original = notion_plan()
    async with runtime() as session:
        run = await session.get(WorkflowRun, "run")
        ...
        transition_run(
            run,
            RunStatus.waiting_for_action,
            reason="test_fixture_identical_repair",
            actor="test",
            dispatch=None,
        )
        step = await session.get(RunStep, "step")
        step.consequential = False
        step.tool_slug = "notion"
        step.operation = "notion.search"
        step.step_key = "find"
        step.arguments = {"query": "report"}
        ...
        session.add(
            AuditEvent(
                ...
                actor="executor",
                event_type="step.recovery_exhausted",
                payload={"step_id": "step", "error": "Search returned no results"},
            )
        )
        session.add(
            StepAttempt(
                ...
                tool_slug="notion",
                operation="notion.search",
                error="[provider_unavailable] Search returned no results",
            )
        )
        await session.commit()

    async def identical(*_args):
        return StepRepair(
            tool_slug="notion",
            operation="notion.search",
            arguments={"query": "report"},
            reason="Try the same search again",
        )

    monkeypatch.setattr(replanning, "_run", identical)
    ...
    async with runtime() as session:
        ...
        event = await session.scalar(
            select(AuditEvent).where(AuditEvent.event_type == "run.replan_no_change")
        )
